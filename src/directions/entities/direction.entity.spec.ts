...
describe('Direction Entity', () => {
  const getTable = () =>
    getMetadataArgsStorage().tables.find((t) => t.target === Direction);

  const getColumns = () =>
    getMetadataArgsStorage().columns.filter((c) => c.target === Direction);

  const getColumn = (name: string) =>
    getColumns().find((c) => c.propertyName === name);

  it('should be mapped to table "directions"', () => {
    const table = getTable();
    expect(table).toBeDefined();
    expect(table!.name).toBe('directions');
  });
